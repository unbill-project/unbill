...
`bill_editor.rs` owns the shared bill editor draft model. Custom share weights are held as raw text while a form is open. Preview helpers parse drafts tolerantly, and the save-request builder performs the strict amount, participant, and share-weight validation used at submission.
