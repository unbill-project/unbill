...
/// The iroh NodeId of a device — a 32-byte Ed25519 public key.
///
/// Stored in the Automerge document as the canonical base32-hex string
/// produced by `iroh::NodeId`'s `Display` impl.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct NodeId(iroh::NodeId);

impl NodeId {
    pub fn from_node_id(id: iroh::NodeId) -> Self {
        Self(id)
    }

    pub fn as_node_id(self) -> iroh::NodeId {
        self.0
    }
}
...
impl FromStr for NodeId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        iroh::NodeId::from_str(s).map(Self)
    }
}
...
// Stored as the iroh NodeId string in the Automerge document.
...
impl autosurgeon::Hydrate for NodeId {
    fn hydrate<'a, D: ReadDoc>(
        ...
        s.parse::<Self>()
            .map_err(|e| HydrateError::unexpected("iroh NodeId string", e.to_string()))
    }
}
