...
**Users:** add a user directly by user ID and display name; list all users. Users are named people in a ledger only — they carry no device binding. Users are append-only and may not be removed. The full invite/join flow (out-of-band token, join URL) is deferred to M4.
