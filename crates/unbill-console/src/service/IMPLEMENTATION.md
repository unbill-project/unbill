...
`inner.rs` contains `UnbillConsole`, `ServiceEvent`, and the helpers for device-local metadata. Opening the service loads or creates the device key and initializes the event broadcaster.
