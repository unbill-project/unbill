...
async fn handle_key<S>(key: KeyEvent, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn handle_ledger_key<S>(key: KeyEvent, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn handle_bills_key<S>(key: KeyEvent, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn handle_editor_key<S>(key: KeyEvent, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn try_confirm_editor<S>(state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn execute_action<S>(action: PopupAction, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
async fn open_settings_popup<S>(tab: TopTab, state: &mut AppState, svc: &Arc<UnbillService<S>>)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
pub async fn refresh_ledgers<S>(svc: &Arc<UnbillService<S>>, state: &mut AppState)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
pub async fn refresh_bills<S>(svc: &Arc<UnbillService<S>>, state: &mut AppState)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
pub async fn refresh_users<S>(svc: &Arc<UnbillService<S>>, state: &mut AppState)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
...
pub async fn refresh_settlement<S>(svc: &Arc<UnbillService<S>>, state: &mut AppState)
where
    S: LockableStore + Send + Sync + 'static,
    for<'a> S::Guard<'a>: Send,
{
